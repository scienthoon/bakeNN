...
Status: host implementation baseline complete; not a target-performance claim
...
| Compute ops | Implemented and host-tested | Conv2D including groups, DepthwiseConv2D, Conv1D including groups, grouped ConvTranspose2D, and FullyConnected reference/portable-C paths. P2 also has generic specializations and a cross-compiled Cortex-M4 DSP family, but no physical target-performance evidence. |
...
| Pool, tensor, shape and output | Implemented and host-tested | 2D/1D Avg/MaxPool, zero Pad2D, spatial/time ReduceMean, Reshape/Flatten/Squeeze/Unsqueeze views, static Slice/Crop, Concatenate, static nearest/Q15-bilinear Resize2D, and BakeNN Q15-LUT Softmax. Softmax is explicitly not TFLite bit-exact. |
...
| MobileNetV3 / EfficientNet-Lite / U-Net expansion | Implemented and host-tested | Compact complete models cover SE broadcast, HardSigmoid/HardSwish, ReLU6 MBConv, skip concat, grouped ConvTranspose2D, static crop and bilinear resize through sanitized generated C. Unmodified torchvision MobileNetV3-small/large and EfficientNet-B0 (a SiLU/SE semantic superset of Lite) reach generated artifacts at 32x32. Pretrained dataset accuracy and target fit remain unmeasured. |
...
| TFLM comparison harness | Skeleton implemented | Offline protocol, versioned result shape, unmeasured example and dependency-free validator exist. No BakeNN-versus-TFLM performance numbers have been measured. |
| Physical MCU benchmark | **Blocker** | Final ELF Flash, full peak SRAM, init cycles, median/p95 inference cycles, energy if claimed, and output comparison remain unmeasured. |
...
1. Freeze a real target/toolchain/TFLM revision and implement the target-side
   cycle, stack-watermark and output-capture shims described in
   `benchmarks/tflm_compare/README.md`.
2. Produce target ELF/map and raw-run evidence for both implementations; do not
   publish performance claims until every required metric is measured under the
   same protocol.
3. Re-run the clean-wheel, compiler/sanitizer and public-API gates after the
   target backend and comparison integration freeze.

The compiler baseline is usable for the declared host-tested surface. A public
claim that it beats TFLM remains blocked until the physical-target evidence
above exists.
