...
| Fully-quantized TFLite import | Implemented for the declared v1 subset | Optional host-only schema-v3 importer for strict static INT8 Conv/Depthwise/FC/Add/Avg/MaxPool/Reshape/Pad graphs. Exact operator versions, per-axis qparams, bias scales and fused activations are validated. TFLite AveragePool retains its separately versioned raw-code rounding profile, and LiteRT BUILTIN_REF, Python and generated C differential gates are byte-exact. Unsupported semantics fail closed. |
...
| Wheel/clean install release gate | Passed locally for `1.0.0.dev0`; CI encoded | The v1 wheel and sdist build cleanly, the wheel installs into a fresh venv, optional frontends are packaged, and generated model/weights/kernel translation units compile from `site-packages`; the release workflow repeats this gate before publication. |
